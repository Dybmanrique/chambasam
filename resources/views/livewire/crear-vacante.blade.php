    ...
    <form action="" class="md:w-1/2 space-y-5" wire:submit.prevent='crearVacante' novalidate>
        ...
        <div>
            ...
            <x-text-input wire:model="imagen" id="imagen" class="block mt-1 w-full" type="file" accept="image/*" required/>
            <div class="my-5">
                @if ($imagen)
                    Imagen: <img class="rounded" src="{{ $imagen->temporaryUrl() }}" alt="">
                @endif
            </div>
            <x-input-error :messages="$errors->get('imagen')" class="mt-2" />
        </div>
        <x-primary-button class="ms-3 ml-0">{{ __('Crear vacante') }}</x-primary-button>
    </form>
